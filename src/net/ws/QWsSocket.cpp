...
// moved from functions.h to reduce file clutter
bool rand2();
...
void QWsSocket::disconnectFromHost(const QString & reason)
{
	QWsSocket::close(CloseStatusCode::CloseNormal, reason);
}
...
qint64 QWsSocket::sendTextMessage(const QString& string)
{
	if (_version == WS_V0)
	{
...
qint64 QWsSocket::sendBinaryMessage(const QByteArray& byteArray)
{
...
void QWsSocket::processDataV0()
{
	...
	{
		emit textReceived(QString::fromUtf8(currentFrame));
		currentFrame.clear();
	}
...
void QWsSocket::handleData()
{
	...
	{
		emit binaryReceived(currentData);
		currentData.clear();
		return;
	}
	if (currentDataOpcode == OpText)
	{
		emit textReceived(QString::fromUtf8(currentData));
		currentData.clear();
		return;
	}
}
